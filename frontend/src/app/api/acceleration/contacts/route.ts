...
import { ContactsRequestSchema } from '@/api_contracts/acceleration/contactsContracts';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = ContactsRequestSchema.safeParse(body);
    if (!parsed.success) {
    ...
    const result = await AccelerationService.suggestContacts({
      userId: auth.userId,
